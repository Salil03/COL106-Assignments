...
def add_token(token, value_stack, operator_stack):
    if token == 'X':
        ...
        operator_stack.push('(')
        value_stack.push([0, 0, 0, 0])  # add further statements to this value
    elif token == ')':
        while operator_stack.top() != '(':  # apply all operations between brackets
            apply_operation(value_stack, operator_stack)
        operator_stack.pop()
    elif token == '+' or token == '-':
        # only apply operators wither higher or equal precedence till first bracket
...
def findPositionandDistance(P):
    ...
    while i < length:
        if P[i].isdigit():
            ...
            add_token(int(P[i:j]), value_stack, operator_stack)
